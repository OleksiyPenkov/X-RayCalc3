...
- `5d3fb22` (3.9.2.970) — registered 2026-09-22 for exp-03 segment 25 on (the P2-08 curve notice): the launcher path `_Out\BIN\XRC_MCP.exe` was rebuilt 2026-09-21 17:53 (sha256 97de1526…), probed over stdio before launch (`describe_server` `5d3fb22`, 17 tools, no `-dirty`). Since beb40c8: dc3b051 result-JSON rounding through `Format('%.6g')` so a converged job with a |value| < 1e-15 (cubic profile coefficient) no longer ends failed, 5d3fb22 inverted layer ranges (min > max) refused by the engine and listed by `load_project` in `inverted_ranges` (fit_xrr bounds were already validated). Neither changes what a fit computes. 2f92b9b `scale_solve` (2026-09-22) is NOT in this binary. The loop agent was told of the rebuild and both changes in curve notice 25.
- `e9ad011` = RELEASE 3.9.2.970 (tag v3.9.2.970, sha256 b65b1978...cbde) — registered 2026-09-22 for exp-03 segment 26 on (the v5 refit intervention): the launcher path was replaced by the released, gate-verified binary at 15:19 (peer message); probed over stdio before launch (describe_server e9ad011, 3.9.2.970, 18 tools). Since 5d3fb22: 2f92b9b/c9113ce `scale_solve` default on (solved scale inside the objective, window 0.2; result fields scale_solve, chi2_scale, scale_ratio, scale_solved, scale_clamped, scale_start_ratio), 9b1106c..579f6e0 `assess_xrr` (tool 18), befc463..2f46efa XRDML import in the inbox and GUI (file wavelength wins, normalised to 1 at max), f20e32c B4C Henke table normalised (bulk 2.52). Gate caveat (peer): smooth passes > 0 drift ~1e-4 in chi2 vs 3.9.1.950; smoothing off = bit-identical. The loop agent was told items 1-3 in refit notice 26, with the v5.0 procedure; the v5 text does not mention scale_solve (gap reported to the skill session), so the agent reports chi2_scale/scale_ratio/scale_clamped and treats chi2 verdicts as provisional.
