...
| 34 | Remove dead `GetCurrentLayerData` getter — empty stub, only setter is used. Changed `LayerData` to write-only property (`unit_XRCStructure.pas`) | 🟢 S | ✅ Done |
...
| 43 | Remove misleading `TConfig.Create`/`FreeAndNil` — all state is `class var`, instance does nothing (`frm_Main.pas`) | 🟢 S | ✅ Done |
